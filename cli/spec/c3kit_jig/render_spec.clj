...
(describe "c3kit-jig.render"
  ...
  (it "render! invokes c3kit-template.bb hook when :hook? true, then deletes hook + context"
    (let [stage    (str (fs/create-temp-dir))
      ...
      (spit (fs/file (fs/path stage "c3kit-template.bb"))
            "(require '[babashka.fs :as fs])
             (let [dir (first *command-line-args*)
                   ctx (slurp (str dir \"/.c3kit-jig-context.edn\"))]
               (spit (str dir \"/hook-ran.txt\") ctx))")
      (spit (fs/file (fs/path stage "c3kit-template.edn")) (pr-str manifest))
      ;; meta-test for the template's own hook — must not ship in scaffold output
      (fs/create-dirs (fs/path stage "spec"))
      ...
      (should-not (fs/exists? (fs/path stage ".c3kit-jig-context.edn")))
      (should-not (fs/exists? (fs/path stage "spec" "hook_test.bb")))
      (fs/delete-tree stage)))
  ...
  (it "render! deletes <ns-token>/<feature> directories and single-file features when feature off"
    (let [stage (str (fs/create-temp-dir))]
      ...
      (fs/create-dirs (fs/path stage "spec" "cljc" "acme"))
      (spit (fs/file (fs/path stage "spec" "cljc" "acme" "markdownc_spec.cljc")) "(ns acme.markdownc-spec)")
      ...
      (should-not (fs/exists? (fs/path stage "spec" "cljc" "my_app" "markdownc_spec.cljc")))
  ...
  (it "render! leaves a secret placeholder referenced inside a feature marker's `= code` clause untouched by secret substitution, deferring to the token pass"
    (let [stage (str (fs/create-temp-dir))]
      (fs/create-dirs (fs/path stage "src" "acme"))
      (spit (fs/file (fs/path stage "src" "acme" "config.clj"))
      ...
      (r/render! stage
                 (edn/read-string (slurp (fs/file (fs/path stage "c3kit-template.edn"))))
                 "my-app" {:auth true} {} "0.1.0-SNAPSHOT")
      (let [config (slurp (fs/file (fs/path stage "src" "my_app" "config.clj")))]
        (should (re-find #"\(env/env \"MY_APP_JWT_SECRET\"\)" config))
        (should-not (re-find #"[0-9a-f]{64}" config)))
      (let [env (slurp (fs/file (fs/path stage ".env")))]
        (should (re-find #"MY_APP_JWT_SECRET=[0-9a-f]{64}" env)))
      (fs/delete-tree stage)))

  (it "write-env! writes .env with placeholder=value lines"
    (let [dir (str (fs/create-temp-dir {:prefix "env-test-"}))]
      (#'r/write-env! dir {"ACME_JWT_SECRET" "deadbeef"})
      (should= "ACME_JWT_SECRET=deadbeef\n" (slurp (str (fs/path dir ".env"))))
      (fs/delete-tree dir)))

  (it "write-env! is a no-op when secret-map is empty"
    (let [dir (str (fs/create-temp-dir {:prefix "env-test-"}))]
      (#'r/write-env! dir {})
