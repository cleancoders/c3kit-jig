(ns acme.sandbox.core
  (:require [acme.layouts :as layouts]
...
(defn handler [request]
  (let [ns-name  (-> (:uri request)
                     (subs 1)
                     (str/replace "/" "."))
        var-name (str "acme." ns-name "/render")
...
(defn sandbox-page-names []
  (let [sandbox-files    (util/clj-files-in "spec/clj/acme/sandbox")
        sandbox-ns-names (map util/filename->ns sandbox-files)
        short-names      (map #(str/replace % "spec.clj.acme.sandbox." "") sandbox-ns-names)]
