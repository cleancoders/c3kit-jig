...
(defn generate-prerender-requires!
  ([namespaces] (generate-prerender-requires! namespaces "dev/acme/prerender_pages.cljs"))
   ...
   (let [requires (str/join "\n            " (map #(str "[" % "]") (sort namespaces)))]
     (spit output-path
           (str ";; Auto-generated by acme.compile-cljs — do not edit\n"
                "(ns acme.prerender-pages\n"
                "  (:require " requires "))\n")))))

(defn- patch-prerender! []
  (let [f (io/file "resources/prerender/prerender.js")]
    (when (.exists f)
      (let [preamble (slurp (io/file "dev/acme/prerender_preamble.js"))
            content  (slurp f)
            patched  (-> content
